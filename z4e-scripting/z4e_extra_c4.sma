...
#define WEAPON_SECRETCODE 1234561

#define WEAPON_ANIMEXT "knife"
#define WEAPON_HUD "weapon_c4"

#define C4_CLASSNAME "z4e_c4"
#define C4_DAMAGE 700.0
#define C4_DAMAGE2 1300.0
#define C4_RADIUS 180.0

new const WEAPON_MODELS[][] = 
{
    "models/z4e/v_morec4.mdl",
    "models/z4e/x_detonator.mdl",
    "models/z4e/x_detonator.mdl",
    "models/z4e/x_morec4.mdl",
    "sprites/fexplo.spr",
    "sprites/eexplo.spr"
}
...
{
    "c4/plant.wav",
    "c4/explode_close.wav"
}

new const WEAPON_SPRITES[][] =
{
    "sound/weapons/detpack_safety.wav",
    "sound/weapons/detpack_trigger.wav"
...
new g_iItemID
new g_bitsHasWeapon, g_iAmmo[33]
...
public plugin_init() 
{
    ...
    g_iItemID = z4e_extra_item_register("C4遥控炸药包", 500)
}
...
public z4e_fw_extra_select_post(id, iItem)
{
    ...
    {
        if(BitsGet(pev(id, pev_weapons), CSW_C4))
...
public fw_SetModel(iEntity, szModel[])
{
    ...
    if(equal(szModel, OLD_W_MODEL))
    {
        ...
        {
            // W模型
            set_pev(pItem, pev_iuser1, g_iAmmo[id])
            g_iAmmo[id] = 0
            ...
            while (pev_valid(pItem))
            {
                ...
                if (equal(szClassname, "weapon_c4"))
                {
                    if(pev(pItem, pev_impulse) == WEAPON_SECRETCODE)
                    {
                        ...
                        {
                            g_iAmmo[pHit] += pev(pItem, pev_iuser1)
                            set_pev(pItem, pev_iuser1, 0)
                            UpdateAmmo(pHit)
                        }
                        else
                        {
...
public HamF_Item_AddToPlayer_Post(this, id)
{
    ...
    {
        BitsSet(g_bitsHasWeapon, id)
        g_iAmmo[id] = pev(this, pev_iuser1)
...
public HamF_Item_PostFrame(this)
{
    ...
    if(pev(id, pev_button) & IN_ATTACK2 && get_pdata_float(this, m_flNextSecondaryAttack, OFFSET_LINUX_WEAPONS) <= 0.0 && !get_pdata_int(this, m_fInReload, OFFSET_LINUX_WEAPONS))
    {
...
public HamF_Weapon_SecondaryAttack(this)
{
    ...
    UTIL_SendWeaponAnim(id, ANIM_SETOFF)
    ...
    while(pev_valid((pEntity = engfunc(EngFunc_FindEntityByString, pEntity, "classname", C4_CLASSNAME))))
    {
        if(pev(pEntity, pev_owner) != id)
            continue;
        Make_Explosion(pEntity)
    }
    
    set_pdata_float(this, m_flNextPrimaryAttack, 1.0)
    set_pdata_float(this, m_flNextSecondaryAttack, 1.0)
    set_pdata_float(this, m_flTimeWeaponIdle, 0.9)
...
public Create_Grenade(this)
{
    ...
    UTIL_SendWeaponAnim(id, ANIM_PLANT)
    ...
    static Float:vecOrigin[3], Float:vecVelocity[3], Float:vecTargetOrigin[3]
    ...
    set_pdata_float(this, m_flNextPrimaryAttack, 1.0)
    set_pdata_float(this, m_flNextSecondaryAttack, 1.0)
    set_pdata_float(this, m_flTimeWeaponIdle, 2.7)
}
...
public HamF_C4_Touch(iEntity, pHit)
{
    ...
    new Float:vecVelocity[3], Float:vecOrigin[3]
    pev(iEntity, pev_velocity, vecVelocity)
    pev(iEntity, pev_origin, vecOrigin)
    
    new Float:vecEnd[3]
    xs_vec_add(vecOrigin, vecVelocity, vecEnd)
    
    new ptr = create_tr2()
    engfunc(EngFunc_TraceLine, vecOrigin, vecEnd, IGNORE_MONSTERS, iEntity, ptr)
    new Float:vecPlaneNormal[3], Float:vecAngles[3]
    ...
    set_pev(iEntity, pev_v_angle, vecPlaneNormal)
    ...
    if(!z4e_team_get_user_zombie(id))
    {
        ...
        while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecOrigin, C4_RADIUS)) && pev_valid(pEntity))
        {
            ...
            if(is_user_alive(pEntity) && z4e_team_get_user_zombie(pEntity))
                z4e_knockback_set(pEntity, id, 800.0, 800.0, 800.0, 800.0, 0.5, Z4E_KNOCKBACK_IGNORECHECK|Z4E_KNOCKBACK_IGNOREABILITY|Z4E_KNOCKBACK_DRAWANIM)
        }
    }
    if(pev_valid(iEnt))
