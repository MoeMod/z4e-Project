...
new const ELEMENT_WIND_MODEL[] = "sprites/z4e/ef_aircyclone.spr"
new const ELEMENT_WIND_TORNADO_MODEL[] = "models/z4e/ef_hurricane2.mdl"
new const ELEMENT_WIND_SOUND[] = "z4e/skill/airburster_shoot.wav"
new const ELEMENT_WIND_CLASSNAME[] = "z4e_element_wind"

new const ELEMENT_GRAVITY_MODEL[] = "sprites/z4e/ef_teleportzombie.spr"
...
ElementWind_RadiusAttack(id)
{
	...
	while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecOrigin, 675.0)) && pev_valid(pEntity))
	{
		if(!is_user_alive(pEntity) || !z4e_team_get_user_zombie(pEntity))
			continue;
		z4e_knockback_set(pEntity, id, 800.0, 1000.0, 650.0, 500.0, 0.7, Z4E_KNOCKBACK_DRAWANIM);
	}
...
ElementWind_ShootDamage(id)
{
	...
	while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecSrc, flRange)) != 0)
	{
		...
		{
			if(g_iDifficulty == DIFF_HARD)
				z4e_knockback_set(pHit, id, 800.0, 1000.0, 650.0, 500.0, 0.9, Z4E_KNOCKBACK_DRAWANIM)
			else
				z4e_knockback_set(pHit, id, 800.0, 1000.0, 650.0, 500.0, 0.7, Z4E_KNOCKBACK_DRAWANIM)
		}
		free_tr2(ptr)
	}
